...
const credits = async () => {
  ...
  await page.evaluate(() => {
    ...
    await page.waitForSelector('img[src="/icons/Theme=light, Name=card-duotone.svg"]');
    ...
    await page.waitForSelector('input[placeholder="Enter Phone Number"][type="tel"]');
    await page.type('input[placeholder="Enter Phone Number"][type="tel"]', "1234567890");
   ...
   // Attempt to find the "Buy Credits" button with retry
    ...
    while (attempt < maxAttempts && !buttonFound) {
      ...
      try {
        ...
        console.log(`Attempt ${attempt} to find 'Buy Credits' button failed. Retrying...`);
        delay(1);
      }
    }
    ...
    if (!buttonFound) {
      ...
      throw new Error("Buy Credits button not found after multiple attempts. Screenshot saved.");
    }

    // Wait for a response or page load
    delay(3);
    console.log("Credits Successful!");
